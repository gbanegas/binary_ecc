...
ff_element ff_zero = {0, 0, 0, 0, 0, 0, 0, 0};
ff_element ff_one = {1, 0, 0, 0, 0, 0, 0, 0};
...
void ff_mul_lrcomb_w4(ff_element a, ff_element b, ff_element c) {
    ...
    if (base[1 * BLOCKS + BLOCKS - 1] & 256) {
        base[2 * BLOCKS + BLOCKS - 1] &= 511;
        base[2 * BLOCKS + 0] ^= 1;
        base[2 * BLOCKS + 2] ^= 1024;
    }
    ...
    if (base[2 * BLOCKS + BLOCKS - 1] & 256) {
        base[4 * BLOCKS + BLOCKS - 1] &= 511;
        base[4 * BLOCKS + 0] ^= 1;
        base[4 * BLOCKS + 2] ^= 1024;
    }
    ...
    if (base[4 * BLOCKS + BLOCKS - 1] & 256) {
        base[8 * BLOCKS + BLOCKS - 1] &= 511;
        base[8 * BLOCKS + 0] ^= 1;
        base[8 * BLOCKS + 2] ^= 1024;
    }
...
void ff_read(ff_element a) {
    scanf("%x %x %x %x %x %x %x %x", a->data + 7, a->data + 6, a->data + 5, a->data + 4, a->data + 3, a->data + 2,
          a->data + 1, a->data + 0);

    assert(!(a->data[BLOCKS - 1] & !511));
    a->data[BLOCKS - 1] &= 511;
}

void ff_sread(const char *s, ff_element a) {
    sscanf(s, "%x %x %x %x %x %x %x %x", a->data + 7, a->data + 6, a->data + 5, a->data + 4, a->data + 3, a->data + 2,
           a->data + 1, a->data + 0);
...
void ff_print(ff_element a) {
    printf("[%x %x %x %x %x %x %x %x]", a->data[7], a->data[6], a->data[5], a->data[4], a->data[3], a->data[2],
           a->data[1], a->data[0]);
